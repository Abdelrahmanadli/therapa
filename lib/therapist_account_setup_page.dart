...
class TherapistAccountSetupPage extends StatefulWidget {
  ...
  @override
  State<TherapistAccountSetupPage> createState() => _TherapistAccountSetupPageState();
}
...
class _TherapistAccountSetupPageState extends State<TherapistAccountSetupPage> {
    ...
    try {
      ...
      if (doc.exists) {
        setState(() {
          ...
          clinicAddressTextController.text = doc['clinicAddress'] as String? ?? '';
          subscriptionPriceTextController.text = (doc['subscriptionPrice'] as num?)?.toString() ?? '';
  ...
  String? _textFieldValidator(String? val, String fieldName, {bool isRequired = true}) {
  ...
  Future<void> _saveTherapistDetails() async {
    ...
    if (!_formKey.currentState!.validate()) {
      _showAlertDialog('Validation Error', 'Please correct the errors before saving.');
      return;
    }

    if (_selectedSpecialties.isEmpty) {
      _showAlertDialog('Validation Error', 'Please select at least one specialty.');
      return;
    }

    if (currentUser == null) {
      _showAlertDialog('Error', 'No authenticated user found. Please log in again.');
      return;
    }

    try {
      await FirebaseFirestore.instance.collection('therapists').doc(currentUser!.uid).update({
        ...
        'specialties': _selectedSpecialties, // Save list of specialties
        'subscriptionPrice': double.parse(subscriptionPriceTextController.text.trim()), // Save as number
        'isSetupComplete': true, // Mark setup as complete
        'lastUpdated': FieldValue.serverTimestamp(), // Optional: add a timestamp
      });
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    validator: (value) => _textFieldValidator(value, 'Education'),
                  ),
                  ...
                  TextFormField(
                    ...
                    style: GoogleFonts.inter(color: primaryText),
                    validator: (value) => _textFieldValidator(value, 'Clinic Address'),
                  ),
                  const SizedBox(height: 16.0),
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      if (_autovalidateMode == AutovalidateMode.always && _selectedSpecialties.isEmpty)
                        Padding(
