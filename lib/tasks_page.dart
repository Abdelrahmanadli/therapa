...
import 'package:therapa/add_task_dialog.dart'; // Adjust path if needed
// Import your new TaskDetailsPage
import 'package:therapa/task_details_page.dart'; // Adjust path if needed
...
const Color accentGreen = Color(0xFFE0FFEA); // For selected calendar day background
const Color alternateColor = Color(0xFFE0E0E0); // Light grey for borders and inactive elements
...
class _TasksPageState extends State<TasksPage> {
  DateTime _selectedDay = DateTime.now(); // Initially selected day in the calendar
  ...
  String _getMonthYear(DateTime date) {
    return DateFormat('MMMM yd').format(date); // Changed to 'yyyy' for consistency
  }
  ...
  Future<void> _toggleTaskCompletion(DocumentSnapshot taskDoc, bool currentStatus) async {
    try {
      await FirebaseFirestore.instance.collection('tasks').doc(taskDoc.id).update({
        'completed': !currentStatus,
      });
      print('Task ${taskDoc.id} completion status toggled to ${!currentStatus}');

      // Handle recurrence logic (create new task for next applicable day if it was just marked complete)
      if (taskDoc['isRecurring'] == true && !currentStatus) { // If it was recurring AND just marked marked complete
        final List<int> repeatDays = List<int>.from(taskDoc['repeatDays'] ?? []);
        ...
        if (!foundNextDayInCurrentWeek && repeatDays.isNotEmpty) {
          ...
          if (currentWeekday == DateTime.sunday) { // If current day is Sunday (7), daysToEndOfCurrentWeek is 0 from Sunday to Saturday.
            daysToEndOfCurrentWeek = 0;
          }

          // Total days to add to get to the first repeat day of the next week
          final int daysToAdd = daysToEndOfCurrentWeek + firstRepeatDayOfNextWeek;
          nextOccurrenceDate = taskDate.add(Duration(days: daysToAdd));
        }
        ...
        if (nextOccurrenceDate != null) {
          ...
          final DateTime normalizedNextOccurrenceDate = DateTime(nextOccurrenceDate.year, nextOccurrenceDate.month, nextOccurrenceDate.day);
          ...
          final existingTasks = await FirebaseFirestore.instance
              ...
              .where('taskDate', isEqualTo: Timestamp.fromDate(normalizedNextOccurrenceDate))
              .limit(1)
              .get();
          ...
          if (existingTasks.docs.isEmpty) {
            ...
            await FirebaseFirestore.instance.collection('tasks').add({
              ...
              'taskDate': Timestamp.fromDate(normalizedNextOccurrenceDate), // Save normalized date
              'isRecurring': true,
              'repeatDays': repeatDays, // Pass the same repeat days to the new instance
              'completed': false, // New recurring task starts incomplete
              'userRef': userRef,
              'created_at': FieldValue.serverTimestamp(),
            });
            print('Created new recurring task for ${DateFormat('MMM d,EEEE').format(normalizedNextOccurrenceDate)}');
          } else {
            print('Task for ${DateFormat('MMM d,EEEE').format(normalizedNextOccurrenceDate)} already exists.');
          }
        } else {
  ...
  Widget build(BuildContext context) {
    ...
    final DateTime endOfDay = startOfDay.add(const Duration(days: 1)).subtract(const Duration(microseconds: 1));
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            await showModalBottomSheet(
              context: context,
              isScrollControlled: true, // Allows content to be full height if needed by keyboard
        ...
        body: Column(
          ...
          children: [
            ...
            Container(
              width: double.infinity,
              height: MediaQuery.sizeOf(context).height * 0.12, // Approximate height
              ...
              child: Padding(
                padding: const EdgeInsetsDirectional.fromSTEB(0.0, 40.0, 0.0, 0.0), // Padding for status bar
                child: Row(
                  mainAxisSize: MainAxisSize.max,
                  ...
                  children: [
                    Align(
                      ...
                      child: InkWell( // Custom back button
                        ...
                        onTap: () {
                          Navigator.pop(context); // Go back to the previous page (e.g., HomePage)
                        },
                        child: Container(
                          width: 40.0, // Size of the tappable area
                          height: 40.0,
                          margin: const EdgeInsets.only(left: 10.0), // Spacing
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(8.0),
                    ...
                    Expanded(
                      child: Align(
                        ...
                        child: Padding(
                          padding: const EdgeInsetsDirectional.fromSTEB(0.0, 0.0, 40.0, 0.0), // Push title slightly left
                          child: Text(
                            'Tasks',
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          IconButton(
                            icon: const Icon(Icons.calendar_today, color: secondaryText, size: 20.0),
                            onPressed: () async {
                                  ...
                                  return Theme(
                                    data: ThemeData.light().copyWith(
                                      colorScheme: ColorScheme.light(
                                        ...
                                        onPrimary: cardBackground, // Header text color
                                        surface: cardBackground, // Dialog background
                          ...
                          IconButton(
                            icon: const Icon(Icons.chevron_left, color: secondaryText, size: 24.0),
                            onPressed: () {
                              setState(() {
                                _selectedDay = _selectedDay.subtract(const Duration(days: 7)); // Go back a week
                              });
                            },
                          ),
                          IconButton(
                            icon: const Icon(Icons.chevron_right, color: secondaryText, size: 24.0),
                            onPressed: () {
                              setState(() {
                                _selectedDay = _selectedDay.add(const Duration(days: 7)); // Go forward a week
                              });
                            },
                          ),
                        ],
                  ...
                  Row(
                    ...
                    children: List.generate(7, (index) {
                      ...
                      if (_selectedDay.weekday == DateTime.sunday) {
                        startOfWeek = _selectedDay.subtract(const Duration(days: 6));
                      }
                      ...
                      return Expanded(
                        child: InkWell(
                          onTap: () {
                            setState(() {
                              _selectedDay = currentDay; // Update selected day on tap
                            });
                          },
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: isSelected ? primaryGreen : Colors.transparent, // Green background for selected
                              borderRadius: BorderRadius.circular(8.0),
                            ),
                            child: Column(
                              children: [
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance
                    ...
                    .where('taskDate', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDay))
                    .where('taskDate', isLessThanOrEqualTo: Timestamp.fromDate(endOfDay))
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return InkWell( // Make the entire task item tappable
                        onTap: () {
                          // Navigate to TaskDetailsPage, passing the task document
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => TaskDetailsPage(taskDoc: taskDoc),
                            ),
                          );
                        },
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              // Checkbox Area
                              InkWell(
                                onTap: () => _toggleTaskCompletion(taskDoc, isCompleted),
                                child: Container(
                                  width: 24.0,
                                  height: 24.0,
                                  decoration: BoxDecoration(
